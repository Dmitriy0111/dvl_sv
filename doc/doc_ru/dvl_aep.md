...
Данный файл содержит описание класса порта, необходимого для одностороннего обмена между верификационными компонентами совместно с классом dvv_ap (analysis port). Класс наследуется от dvv_bp и является передатчиком данных. Переменная для обмена может иметь системный или пользовательский тип.
...
#### new
Конструктор класса для создания экземпляра dvv_aep.
...
Для добавления порта приёмника (analysis port) необходимо вызвать функцию connect, при этом переменных для обмена должны совпадать. Также к одному экземпляру класса dvv_aep можно подключать несколько dvv_ap:
```verilog
